...
try {
    ...
    // Mail headers
    $fromEmail = $config['EMAIL_USER_FROM'] ?? $config['SMTP_USER'];
    $fromName  = $config['EMAIL_USER_FROM_NAME'] ?? 'Go To Market';
    $mail->setFrom($fromEmail, $fromName);
    if (!empty($config['EMAIL_USER_REPLY_TO'])) {
        $mail->addReplyTo($config['EMAIL_USER_REPLY_TO']);
    }
    ...
    $downloadBase = rtrim((string) env('BASE_URL', ''), '/');
    $downloadUrl = $downloadBase !== ''
        ? "{$downloadBase}/api/uploads/{$language}/{$filename}"
        : "/api/uploads/{$language}/{$filename}";
