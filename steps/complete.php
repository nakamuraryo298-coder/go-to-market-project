 ...
 * ----------------------------------------------------
 */
$language = $_SESSION['lang'] ?? ($_GET['lang'] ?? 'ja');
 ...
 * ----------------------------------------------------
 */
require_once __DIR__ . '/../inc/assessment-scoring.php';
require_once __DIR__ . '/../inc/assessment-question-builder.php';
$session = $_SESSION;
        ...
        <div class="max-w-2xl mx-auto p-6">
            ...
            <div class="space-y-4">
                <button id="get_report"
                    class="w-full py-4 px-6 text-white rounded-lg font-semibold text-lg shadow-lg hover:opacity-90 flex items-center justify-center gap-3"
                    style="background-color: rgb(12, 83, 149)">

                    <span id="btnText"><?= t('assessment.complete.getYourReport'); ?></span>

                </button>


                <button id="back_to_top"
                    class="w-full py-4 px-6 border-2 border-gray-300 text-gray-700 rounded-lg font-semibold text-lg hover:bg-gray-50">
                    <?= t('assessment.complete.backToTop'); ?>
                </button>

                <button id="book_consultation"
                    class="w-full py-4 px-6 border-2 rounded-lg font-semibold text-lg hover:opacity-90"
                    style="border-color: rgb(12, 83, 149); color: rgb(12, 83, 149)">
...
btn?.addEventListener("click", function () {
    ...
    .then(data => {
        ...
        // optional redirect
        window.location.href = "<?= BASE_URL ?>/gtm-assessment?step=success";
    })
    .catch(err => {
    ...
    .finally(() => {
        ...
        // No separate loader element is rendered in the current button markup.
        btnText.innerText = "<?= t('assessment.complete.getYourReport'); ?>";
    });
});
...
document.getElementById("book_consultation")?.addEventListener("click", () => {
    window.open("https://timerex.net/s/contact_d1e0_257d/86059caa", "_blank");
});
</script>
