...
<script>
    ...
    function updateUI() {
        ...
        if (count > 0) {
            ...
            selectedText.innerText = `${count}/5個の課題が選択されました`;
        } else {
    ...
    function prevStep(){
        saveToLocal();
        window.location.href = '<?= BASE_URL ?>/gtm-assessment?step=questions&from=challenges';
    }
    ...
    function saveAndNext(){
        ...
        .then(res => res.json())
        .then(()=>{
            const redirectUrl = '<?= BASE_URL ?>/gtm-assessment?step=complete';
